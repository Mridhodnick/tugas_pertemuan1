...
<li class="nav-item ">
<a class="nav-link" href="{{('/trainers/profile')}}">
            ...
            <div class="card">
              <div class="card-header card-header-info">
                ...
                @if(Auth::user()->id == $question->user->id)
                  ...
                  <form action="{{route('trainers_edit_question')}}" method="post" class="d-inline">
                    @csrf 
                    <input type="number" name="question_id" value="{{$question->id}}" hidden required>
                    <button class="btn btn-primary pull-right" type="submit"> Edit </button>
                  </form>
                  <!-- fitur delete question-->
                  <form action="{{route('trainers_delete_question')}}" method="post" class="d-inline">
                    @csrf
              ...
              <div class="card-body table-responsive">
                <!-- fitur read response -->
                <p><i>All responses to this question are below: </i></p>
                @foreach($question->responses as $response)
                  ...
                  @if(Auth::user()->id == $response->user->id)
                    ...
                    <form action="{{route('trainers_update_response')}}" method="post" class="d-inline">
                      @csrf 
                      <input type="number" name="response_id" value="{{$response->id}}" hidden required>
                      <button class="btn btn-warning" type="submit"> Edit </button>
                    </form>
                    <!-- form delete response -->
                    <form action="{{route('trainers_delete_response')}}" method="post" class="d-inline">
                      @csrf
                ...
                <form action="{{route('trainers_response')}}" method="post">
                  @csrf 
    ...
    <div class="col-md-4">
      <div class="card card-profile">
        ...
        <div class="card-body">
          ...
          <form action="{{route('trainers_ask')}}" method="post">
            @csrf
            <div class="row">
              <div class="col-md-12">
